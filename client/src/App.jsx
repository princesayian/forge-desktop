...
const addCustomRColor=()=>{const h=rCustomHex.trim();if(!h.match(/^#[0-9a-fA-F]{6}$/)&&!h.match(/^#[0-9a-fA-F]{3}$/))return;setRColors(prev=>{if(prev.includes(h)||prev.length>=3)return prev;return[...prev,h];});setRCustomHex("#ffffff");};
  ...
  const generateRecruit=async()=>{
    ...
    const hex=rColors[0]||"#A32D2D";const colorDesc=rColors.length===1?hex:rColors.map((c,i)=>(["primary","secondary","tertiary"][i]||"tertiary")+": "+c).join(", ");
  ...
  const generateStory=async()=>{
    ...
    try{
      const p=await callAI(`Write a Meta AI image prompt for a superhero scene. JSON only, key "prompt".\n\nCast: ${castChars.map(m=>{const s=ageStage(m.age);return`${m.heroName}${m.isVillain?" (VILLAIN)":""}${s?" ["+s+"]":""} (${m.powerFX||m.color+" energy"}, ${m.costumeDesc||"dramatic suit"})`;}).join("; ")}\nScenario: ${scene?.label} — ${scene?.desc}\nTone: ${sTone}\n\nNote: Each character's age stage is noted in brackets — ensure physique, face, and proportions match that life stage.\n\n{"prompt":"4-6 sentence Meta AI image prompt. Include each character with exact visual power effects. Dramatic nighttime setting matching the tone. End with: Cinematic wide shot, all characters visible, high-detail comic book illustration style."}`);
      ...
      {tab==="recruit"&&(<>
        ...
        {!rResult&&((!deepMode&&!profileMode&&rStep===0)||(deepMode&&deepPhase===0)||(profileMode&&profileStep===0))&&(<div style={s.card}>
          ...
          {rColors.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
            {rColors.map((c,i)=><div key={c} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 10px",background:`${c}14`,border:`1px solid ${c}55`,borderRadius:20}}>
              <div style={{width:8,height:8,borderRadius:"50%",background:c}}/>
              <span style={{fontSize:9,color:c,fontFamily:"var(--font-mono)"}}>{["Primary","Secondary","Tertiary"][i]}: {c}</span>
              {i>0&&<button onClick={()=>setRColors(prev=>prev.filter(x=>x!==c))} style={{background:"none",border:"none",cursor:"pointer",color:`${c}99`,fontSize:12,padding:0,lineHeight:1,marginLeft:2}}>×</button>}
            </div>)}
          </div>}
          {deepMode&&(<>
