export default function Tripo3DLauncher({prompt,copied,onCopy}){
  const launch=()=>{
    ...
    try{window.open("https://www.tripo3d.ai/","_blank");}catch(e){}
    onCopy();
  };
  return(<div style={{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}>
    ...
    <button onClick={launch} style={{padding:"5px 14px",background:"rgba(15,110,86,0.12)",border:"1px solid rgba(15,110,86,0.4)",borderRadius:6,cursor:"pointer",fontSize:11,color:"#5DCAA5",fontFamily:"var(--font-mono)",display:"flex",alignItems:"center",gap:5}}>
      <span style={{fontSize:13}}>⬡</span> Copy & Open Tripo3D
    </button>
  </div>);
}
