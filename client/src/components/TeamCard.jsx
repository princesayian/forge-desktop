import { ALIGN_META, TEAM_TYPES, TEAM_RANKS, G } from '../constants/index.js';
    ...
    <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:3}}>
          <div style={{fontSize:13,fontWeight:"bold",color:"var(--text-primary)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{team.name}</div>
          {isActive&&<div style={{fontSize:8,padding:"1px 7px",background:`${team.color}22`,border:`1px solid ${team.color}55`,borderRadius:10,color:team.color,flexShrink:0,fontFamily:"var(--font-mono)"}}>Active</div>}
        </div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {team.nkAlignment&&team.nkAlignment!=="base"&&<><span style={{fontSize:9,color:"var(--text4)"}}>·</span><span style={{fontSize:9,color:am.color,fontFamily:"var(--font-mono)"}}>{am.label}</span></>}
        </div>
      </div>
    </div>
