export default function MetaAILauncher({prompt,label,copied,onCopy}){
  const launch=()=>{
    ...
    try{window.open("https://www.meta.ai/","_blank");}catch(e){}
    onCopy();
  };
  return(<div style={{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}>
    ...
    <button onClick={launch} style={{padding:"5px 14px",background:"rgba(30,144,255,0.12)",border:"1px solid rgba(30,144,255,0.4)",borderRadius:6,cursor:"pointer",fontSize:10,color:"#5EB1FF",fontFamily:"var(--font-mono)",display:"flex",alignItems:"center",gap:5}}>
      <span style={{fontSize:12}}>✦</span> Copy & Open Meta AI
    </button>
  </div>);
}
